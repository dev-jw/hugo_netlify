...
![NFC_Protocol_Stack](https://w-md.imzsy.design/NFC_Protocol_Stack.png)
...
- NDEF Records：NDEF消息中的有效负载记录，对应为 `NFCNDEFPayload`
  ...
  **NFCNDEFPayload解释**
  ...
  ![image-20210323202233044](https://w-md.imzsy.design/image-20210323202233044.png)
  ...
  在 iOS13 之后，对于 `MifareTag` 的标签，可以通过`sendMiFareCommand`或`sendMiFareISO7816Command`对标签的内存进行读写
  ...
  **操作指令**
  ...
  ```swift
   let readBlockCommand: UInt8 = 0x30
   
   let dataOffset: UInt8 = 0xD0
   
   let readCommand = Data([readBlockCommand, dataOffset]) 
   
   tag.sendMiFareCommand(commandPacket: readCommand) { (response: Data, optionalError: Error?) in
  ...
  > 针对于受密码保护的标签，我们可以先通过验证指令，将密码发送给标签进行验证
  >
  > 当密码验证成功后，可以通过读写操作，访问内存块
  ...
  **过程：**
  ...
  ![image-20210304150620691](https://w-md.imzsy.design/image-20210304150620691.png)
  ...
  - `type`等于**U**
    ...
    | Email                    | [mailto:user@example.com](mailto:mailto:user@example.com)    |
    | SMS                      | sms:+14085551212                                             |
    | Telephone                | tel:+14085551212                                             |
    | FaceTime                 | facetime://[user@example.com](mailto:user@example.com)       |
    | FaceTime Audio           | facetime-audio://[user@example.com](mailto:user@example.com) |
    | Maps                     | [http://maps.apple.com/?address=Apple%20Park,Cupertino,California](http://maps.apple.com/?address=Apple Park,Cupertino,California) |
